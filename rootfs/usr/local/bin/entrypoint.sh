...
# @@File             :  entrypoint.sh
# @@Description      :  entrypoint point for theHarvester
...
__heath_check() {
  ...
  start-theHarvester.sh healthcheck || status=$((status + 1))
...
the[hH]arvester | [Hh]arvester)
  shift 1
  __exec_command "/app/theHarvester.py" "$@"
  ;;
